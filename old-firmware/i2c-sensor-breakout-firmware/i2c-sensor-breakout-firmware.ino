/* # i2c Sensor Mux Breakout Board - Firmware - MAIN
- Collect data from a series of Senserion i2c sensors using i2c Mux breakout board
- Output resulting data in CSV format for intake into Data Aquisition Software (LabView, Python, Matlab, etc)
...
|No. |Sensor Name (system)  |	Sensor Type (model) |	Data Returned       |	Units                 |	Max Sample Rate (Hz or every X seconds) | Mux.Channel  |
|--- |---                   |---                  |---                  |---                    |---                                      |---           |
